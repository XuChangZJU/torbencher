...
class TorchAtleastU1dTestCase(TorBencherTestCaseBase):
    ...
    def test_atleast_1d_correctness(self):
        ...
        tensor1 = torch.tensor(random.uniform(0.1, 10.0)) if dim1 == 0 else torch.randn(
            [random.randint(1, 5) for _ in range(dim1)])
        ...
        tensor2 = torch.tensor(random.uniform(0.1, 10.0)) if dim2 == 0 else torch.randn(
            [random.randint(1, 5) for _ in range(dim2)])
