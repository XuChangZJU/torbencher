...
@test_api(torch.sparsebsrtensor)
class TorchSparsebsrtensorTestCase(TorBencherTestCaseBase):
    ...
    block_size = (random.randint(1, 3), random.randint(1, 3)) # blocksize[0] and blocksize[1] should be greater than 0
    dense_dims = (random.randint(1, 3), random.randint(1, 3)) # densesize[0] and densesize[1] should be greater than 0

    # Generate crow_indices
    crow_indices = [0]
    for i in range(batch_size):
        for j in range(nrowblocks):
            crow_indices.append(crow_indices[-1] + random.randint(1, 3)) # Each element should be greater than or equal to the previous element
