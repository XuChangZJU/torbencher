...
class TorchTensorTestCase(TorBencherTestCaseBase):
    ...
    def test_tensor_correctness(self):
        ...
        assert (int_tensor == torch.tensor(int_data,dtype=torch.int32)).all(), "Tensor values do not match expected values"
        # 测试生成多维张量
        ...
        assert torch.allclose(multi_dim_tensor, torch.tensor(multi_dim_data,dtype=torch.float32)), "Tensor values do not match expected values"
