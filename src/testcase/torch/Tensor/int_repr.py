...
@test_api(torch.Tensor.int_repr)
class TorchTensorIntUreprTestCase(TorBencherTestCaseBase):
    ...
    def test_int_repr_correctness(self):
        ...
        quantized_tensor = torch.quantize_per_tensor(torch.randn(input_size), scale=random.uniform(0.1, 1.0),
                                                     zero_point=random.randint(0, 255), dtype=torch.quint8)
