...
@test_api(torch.cudnn_batch_norm)
class TorchCudnnBatchNormTestCase(TorBencherTestCaseBase):
    @test_api_version.larger_than("1.1.3")
    def test_cudnn_batch_norm_correctness(self):
        ...
        result = torch.cudnn_batch_norm(input, running_mean, running_var, weight, bias, training=training, momentum=momentum, eps=eps)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_cudnn_batch_norm_large_scale(self):
