...
class TorchJitIsinstanceTestCase(TorBencherTestCaseBase):
    ...
    def test_isinstance_correctness(self):
        ...
        type_choice = random.choice(['List[torch.Tensor]', 'Dict[str, str]', 'Optional[Tuple[int, str, int]]', 'bool', 'int'])
        ...
        if type_choice == 'List[torch.Tensor]':
            ...
            input_data = {f"key{random.randint(1, 10)}": f"val{random.randint(1, 10)}" for _ in range(random.randint(1, 5))}
