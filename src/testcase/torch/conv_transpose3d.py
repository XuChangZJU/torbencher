...
@test_api(torch.conv_transpose3d)
class TorchConvTranspose3dTestCase(TorBencherTestCaseBase):
    ...
    def test_conv_transpose3d_correctness(self):
        ...
        input = torch.randn(batch, in_channels, random.randint(1, 10), random.randint(1, 10), random.randint(1, 10))
        weight = torch.randn(in_channels, groups, kernel_size, kernel_size, kernel_size)
        bias = torch.randn(out_channels)
        result = torch.conv_transpose3d(input, weight, bias, stride=stride, padding=padding, output_padding=output_padding, groups=groups, dilation=dilation)
        return result
