...
class TorchDistributionsMixturesamefamilyMixturesamefamilyTestCase(TorBencherTestCaseBase):
    ...
    def test_MixtureSameFamily_correctness(self):
        ...
        input_size_mixture_distribution = [num_of_elements_each_dim_mixture_distribution for i in range(dim_mixture_distribution)]
        ...
        probs_mixture_distribution /= probs_mixture_distribution.sum(-1, keepdim=True)  # Normalize to get valid probabilities
        ...
        component_distribution = torch.distributions.Normal(loc=loc_component_distribution, scale=scale_component_distribution)
    
        # Create MixtureSameFamily distribution
        mixture_same_family_distribution = torch.distributions.mixture_same_family.MixtureSameFamily(mixture_distribution, component_distribution)
