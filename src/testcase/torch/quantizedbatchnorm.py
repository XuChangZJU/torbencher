...
@test_api(torch.quantizedbatchnorm)
class TorchQuantizedbatchnormTestCase(TorBencherTestCaseBase):
    ...
    def test_quantized_batch_norm_correctness(self):
        ...
        quantized_input = torch.quantize_per_tensor(input_tensor, scale=input_scale, zero_point=input_zero_point, dtype=torch.quint8)
        ...
        result = torch.quantized_batch_norm(quantized_input, weight, bias, mean, var, eps, output_scale, output_zero_point)
        return result
