...
class TorchAffineGridGeneratorTestCase(TorBencherTestCaseBase):
    @test_api_version.larger_than("1.1.3")
    def test_affine_grid_generator_correctness(self):
        batch = random.randint(1, 10)
        channel = random.randint(1, 10)
        height = random.randint(1, 10)
        width = random.randint(1, 10)
        theta = torch.randn(batch, 2, 3)
        result = torch.affine_grid_generator(theta, torch.Size([batch, channel, height, width]))
        return result

    @test_api_version.larger_than("1.1.3")
    def test_affine_grid_generator_large_scale(self):
