...
class TorchExportModulecallsignatureTestCase(TorBencherTestCaseBase):
    ...
    def test_module_call_signature_correctness(self):
        ...
        in_spec = TreeSpec(None, None, [TensorArgument(torch.randn(random.randint(1, 5), random.randint(1, 5))) for _ in range(num_inputs)])
        out_spec = TreeSpec(None, None, [TensorArgument(torch.randn(random.randint(1, 5), random.randint(1, 5))) for _ in range(num_outputs)])
