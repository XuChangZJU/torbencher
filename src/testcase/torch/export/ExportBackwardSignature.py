...
class TorchExportExportbackwardsignatureTestCase(TorBencherTestCaseBase):
    ...
    def test_ExportBackwardSignature_correctness(self):
        ...
        gradients_to_parameters = {f"grad{i}": f"param{i}" for i in
                                   range(num_gradients_to_parameters)}  # type: Dict[str, str]

        num_gradients_to_user_inputs = random.randint(1, 5)
        gradients_to_user_inputs = {f"grad{i}": f"input{i}" for i in
                                    range(num_gradients_to_user_inputs)}  # type: Dict[str, str]

        loss_output = "loss"  # type: str

        # Create an instance of ExportBackwardSignature
        export_backward_signature = torch.export.ExportBackwardSignature(gradients_to_parameters,
                                                                         gradients_to_user_inputs, loss_output)
