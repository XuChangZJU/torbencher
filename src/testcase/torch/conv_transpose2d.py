...
@test_api(torch.conv_transpose2d)
class TorchConvTranspose2dTestCase(TorBencherTestCaseBase):
    ...
    def test_conv_transpose2d_correctness(self):
        ...
        result = torch.conv_transpose2d(input, weight, bias, stride=stride, padding=padding, output_padding=output_padding, groups=groups, dilation=dilation)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_conv_transpose2d_large_scale(self):
