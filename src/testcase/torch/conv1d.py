...
class TorchConv1dTestCase(TorBencherTestCaseBase):
    ...
    def test_conv1d_correctness(self):
        ...
        weight = torch.randn(channel, groups, kernel_size)
        bias = torch.randn(channel)
        result = torch.conv1d(input, weight, bias, stride=stride, padding=padding, dilation=dilation, groups=groups)
        return result
