...
class TorchBlockdiagTestCase(TorBencherTestCaseBase):
    ...
    tensor_dims = [(random.randint(1, 5), random.randint(1, 5)) if random.random() < 0.5 else (random.randint(1, 5),) if random.random() < 0.5 else (random.randint(1,5),) for _ in range(num_tensors)]
