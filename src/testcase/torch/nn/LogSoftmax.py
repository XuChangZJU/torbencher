...
class TorchLogSoftmaxTestCase(TorBencherTestCaseBase):
    @test_api_version.larger_than("1.1.3")
    def test_logsoftmax_correctness(self):
        input_tensor = torch.randn(random.randint(1, 10), random.randint(1, 10))
        log_softmax = torch.nn.LogSoftmax(dim=1)
        result = log_softmax(input_tensor)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_logsoftmax_large_scale(self):
