...
class TorchLazyConv2dTestCase(TorBencherTestCaseBase):
    ...
    def test_lazyconv2d_correctness(self):
        ...
        lazy_conv2d = torch.nn.LazyConv2d(out_channels, kernel_size, stride=stride, padding=padding)
        result = lazy_conv2d(input_tensor)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_lazyconv2d_large_scale(self):
