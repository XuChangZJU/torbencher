
import torch
import random

from src.testcase.TorBencherTestCaseBase import TorBencherTestCaseBase
from src.util import test_api_version
from src.util.decorator import test_api

@test_api(torch.nn.PairwiseDistance)
class TorchPairwiseDistanceTestCase(TorBencherTestCaseBase):
    @test_api_version.larger_than("1.1.3")
    def test_pairwisedistance_correctness(self):
        input1 = torch.randn(random.randint(1, 10), random.randint(1, 10))
        input2 = torch.randn(random.randint(1, 10), random.randint(1, 10))
        pairwise_distance = torch.nn.PairwiseDistance()
        result = pairwise_distance(input1, input2)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_pairwisedistance_large_scale(self):
        input1 = torch.randn(random.randint(1000, 10000), random.randint(100, 1000))
        input2 = torch.randn(random.randint(1000, 10000), random.randint(100, 1000))
        pairwise_distance = torch.nn.PairwiseDistance()
        result = pairwise_distance(input1, input2)
        return result

