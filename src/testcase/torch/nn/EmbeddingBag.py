...
class TorchNnEmbeddingbagTestCase(TorBencherTestCaseBase):
    ...
    def test_embedding_bag_correctness(self):
        ...
        num_offsets = random.randint(1, input_length)  # Ensure we have at least one and not more offsets than input_length
        offsets = torch.arange(0, input_length + 1, step=(input_length // num_offsets) if input_length > num_offsets else 1, dtype=torch.long)
