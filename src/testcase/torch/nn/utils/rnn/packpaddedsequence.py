...
@test_api(torch.nn.utils.rnn.packpaddedsequence)
class TorchNnUtilsRnnPackpaddedsequenceTestCase(TorBencherTestCaseBase):
    ...
    def test_pack_padded_sequence_correctness(self):
        ...
        lengths = sorted([random.randint(1, input_size[0]) for _ in range(batch_size)], reverse=True) # sorted lengths in decreasing order
