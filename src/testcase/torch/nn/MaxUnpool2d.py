...
class TorchMaxUnpool2dTestCase(TorBencherTestCaseBase):
    ...
    def test_maxunpool2d_correctness(self):
        ...
        indices = torch.randint(0, input_tensor.size(-1), (random.randint(1, 10), random.randint(1, 10), random.randint(1, 10), random.randint(1, 10)))
        max_unpool = torch.nn.MaxUnpool2d(kernel_size, stride=stride, padding=padding)
        result = max_unpool(input_tensor, indices)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_maxunpool2d_large_scale(self):
        ...
        indices = torch.randint(0, input_tensor.size(-1), (random.randint(1000, 10000), random.randint(100, 1000), random.randint(100, 1000), random.randint(100, 1000)))
        max_unpool = torch.nn.MaxUnpool2d(kernel_size, stride=stride, padding=padding)
        result = max_unpool(input_tensor, indices)
        return result
