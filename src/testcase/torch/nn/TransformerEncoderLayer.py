...
class TorchNnTransformerencoderlayerTestCase(TorBencherTestCaseBase):
    ...
    def test_transformer_encoder_layer_correctness(self):
        ...
        batch_first = random.choice([True, False])  # Whether input and output tensors are provided as (batch, seq, feature)
        norm_first = random.choice([True, False])  # Whether layer norm is done prior to attention and feedforward operations
    
        # Create the TransformerEncoderLayer
        encoder_layer = torch.nn.TransformerEncoderLayer(d_model, nhead, dim_feedforward, dropout, 'relu', layer_norm_eps, batch_first, norm_first)
