...
class TorchMaxPool3dTestCase(TorBencherTestCaseBase):
    ...
    def test_maxpool3d_correctness(self):
        ...
        dilation = (random.randint(1, kernel_size[0]), random.randint(1, kernel_size[1]), random.randint(1, kernel_size[2]))
        ...
        max_pool = torch.nn.MaxPool3d(kernel_size, stride=stride, padding=padding, dilation=dilation)
        result = max_pool(input_tensor)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_maxpool3d_large_scale(self):
        ...
        padding = (random.randint(0, kernel_size[0]), random.randint(0, kernel_size[1]), random.randint(0, kernel_size[2]))
        dilation = (random.randint(10, kernel_size[0]), random.randint(10, kernel_size[1]), random.randint(10, kernel_size[2]))
