...
class TorchGaussianNLLLossTestCase(TorBencherTestCaseBase):
    @test_api_version.larger_than("1.1.3")
    def test_gaussiannllloss_correctness(self):
        ...
        gaussian_nll_loss = torch.nn.GaussianNLLLoss()
        result = gaussian_nll_loss(input_tensor, target)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_gaussiannllloss_large_scale(self):
