
import torch
import random

from src.testcase.TorBencherTestCaseBase import TorBencherTestCaseBase
from src.util import test_api_version
from src.util.decorator import test_api

@test_api(torch.nn.MaxPool1d)
class TorchMaxPool1dTestCase(TorBencherTestCaseBase):
    @test_api_version.larger_than("1.1.3")
    def test_maxpool1d_correctness(self):
        kernel_size = random.randint(1, 10)
        stride = random.randint(1, kernel_size)
        padding = random.randint(0, kernel_size)
        dilation = random.randint(1, kernel_size)
        input_tensor = torch.randn(random.randint(1, 10), random.randint(1, 10), random.randint(1, 10))
        max_pool = torch.nn.MaxPool1d(kernel_size, stride=stride, padding=padding, dilation=dilation)
        result = max_pool(input_tensor)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_maxpool1d_large_scale(self):
        kernel_size = random.randint(100, 1000)
        stride = random.randint(10, kernel_size)
        padding = random.randint(0, kernel_size)
        dilation = random.randint(10, kernel_size)
        input_tensor = torch.randn(random.randint(1000, 10000), random.randint(100, 1000), random.randint(100, 1000))
        max_pool = torch.nn.MaxPool1d(kernel_size, stride=stride, padding=padding, dilation=dilation)
        result = max_pool(input_tensor)
        return result

