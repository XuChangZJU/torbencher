...
class TorchNNFunctionalConvTranspose1dTestCase(TorBencherTestCaseBase):
    ...
    def test_conv_transpose1d_common(self, input=None):
        if input is not None:
            result = torch.nn.functional.conv_transpose1d(input[0], input[1], bias=input[2], stride=input[3], padding=input[4], output_padding=input[5], groups=input[6], dilation=input[7])
            return [result, input]
        a = torch.randn(1, 3, 8)
        b = torch.randn(3, 3, 2)
        c = None
