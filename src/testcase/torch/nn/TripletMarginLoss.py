...
class TorchTripletMarginLossTestCase(TorBencherTestCaseBase):
    @test_api_version.larger_than("1.1.3")
    def test_tripletmarginloss_correctness(self):
        ...
        triplet_margin_loss = torch.nn.TripletMarginLoss(margin=margin)
        result = triplet_margin_loss(input_tensor, target)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_tripletmarginloss_large_scale(self):
