...
@test_api(torch.cudnn_is_acceptable)
class TorchCudnnIsAcceptableTestCase(TorBencherTestCaseBase):
    @test_api_version.larger_than("1.1.3")
    def test_cudnn_is_acceptable_correctness(self):
        ...
        result = torch.cudnn_is_acceptable(input, weight, stride=stride, padding=padding, dilation=dilation, groups=groups)
        return result

    @test_api_version.larger_than("1.1.3")
    def test_cudnn_is_acceptable_large_scale(self):
